...
- **Lab 2 — Steps 6–7 (layout):** Step 6 listed the YAML edit as `#` comments inside the command block, so pasting the block ran the test without the edit and `nc` reported `bad address 'db'`. The edit now has its own YAML block and a check that the file saved. Step 7 opened an interactive shell with `docker exec -it … bash`, so the pasted `awslocal` lines never ran; each command now runs through `docker exec` directly.
